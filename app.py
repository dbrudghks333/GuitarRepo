...
def upload_file():
    if request.method == "POST":
        ...
        us["lilypondPath"] = "/root/01_File_Upload/LilyPond/usr/bin/lilypond.exe"
        us[
            "musescoreDirectPNGPath"
        ] = "/root/01_File_Upload/MuseScore 2/bin/MuseScore.exe"
        us["musicxmlPath"] = "/root/01_File_Upload/MuseScore 2/bin/MuseScore.exe"
        ...
        for x in files:
            if not os.path.isdir(x):
                ...
                try:
                    ...
                    conv.write(
                        original_score, fmt="lilypond", fp="score", subformats=["png"]
                    )
                    # original_score.show()
...
def url_check():
    if request.method == "POST":
        DOWNLOAD_FOLDER = "/root/01_File_Upload"
        a = request.form["url"]
    ...
    url = (
        "https://www.akbobada.com/searchAll.html?searchKeyword=%s&searchFlag=10&searchSecod=&viewFlag=null&artistOrderBy=null&searchSelect=null&searchSelectName=null"
        % url_new
    )
    ...
    source = "/root/01_File_Upload/sam122.png"
    destination = "/root/01_File_Upload/static/images/sam122.png"
    shutil.move(source, destination)
    ...
    source = "/root/01_File_Upload/%s.mid" % uploaded_audio
    destination = "/root/01_File_Upload/uploads/%s.mid" % uploaded_audio
    shutil.move(source, destination)
    remove = "/root/01_File_Upload/%s.wav" % uploaded_audio
    os.remove(remove)

    source = "/root/01_File_Upload/%s" % uploaded_audio
    destination = "/root/01_File_Upload/playlist/%s" % uploaded_audio
    shutil.move(source, destination)
...
def list_down():
    file_list = os.listdir(os.path.join(".", "uploads"))
